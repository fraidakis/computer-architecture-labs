...
\definecolor{lightOrangeBg}{RGB}{255,247,237}   % Light orange bg
...
\begin{abstract}
\noindent
This report presents the design and optimization of an FPGA-based image processing accelerator implementing absolute difference, posterization, and sharpening filter operations. Three architectural variants were developed: \textbf{V1} (sequential with 2D buffers), \textbf{V2} (sequential with line buffers), and \textbf{V3} (dataflow streaming). Key findings include a \textbf{64$\times$ theoretical speedup} from V1 to V3 through dataflow parallelism, and identification of memory bandwidth as the primary bottleneck. Multi-bank DDR mapping via \texttt{connectivity.cfg} reduced V3 execution time from 11~$\mu$s to \textbf{5~$\mu$s}, doubling transfer rates from 8.9 to 18.9~GB/s.
\end{abstract}
...
% ---------------------------------------------------------------------------
% V3 ANALYSIS (Compute-Bound vs Memory-Bound)
% ---------------------------------------------------------------------------

\begin{solutionbox}
{\color{warningOrange}\faExclamationTriangle}\hspace{2mm}\textbf{V3 Performance Discrepancy: Compute-Bound vs Memory-Bound}

\vspace{2mm}

\begin{tabular}{@{}lr@{}}
\textbf{HLS Latency:} & $\sim$1,050 cycles \\
\textbf{FPGA Frequency:} & 300 MHz \\
\textbf{Theoretical Time:} & $1050 \div 300\text{ MHz} \approx \mathbf{3.5~\mu s}$ \\
\textbf{Measured Time:} & $\mathbf{11~\mu s}$ \\
\textbf{Discrepancy:} & $\sim$3$\times$ slower than expected \\
\end{tabular}

\vspace{3mm}

{\color{dangerRed}\faBolt}\hspace{2mm}\textbf{Root Cause: Memory Bandwidth Bottleneck}

\vspace{2mm}

The V3 kernel is \textbf{memory-bound}, not compute-bound. The discrepancy arises because the kernel's memory demands exceed what a single DDR bank can provide.
\end{solutionbox}

\begin{solutionbox}
{\color{accentBlue}\faMemory}\hspace{2mm}\textbf{Alveo U200 Memory Specifications}

\vspace{2mm}

\begin{tabular}{@{}ll@{}}
\textbf{Memory Type:} & DDR4 \\
\textbf{Configuration:} & 4 banks $\times$ 16 GB (64 GB Total) \\
\textbf{Max Data Rate:} & 2400 MT/s (MegaTransfers/second) \\
\textbf{Total Bandwidth (All 4 Banks):} & 77 GB/s \\
\end{tabular}

\vspace{2mm}

\textbf{Peak Rate Per Channel:}
$$ \text{Bandwidth} = 2400~\text{MT/s} \times 8~\text{bytes} = 19,200~\text{MB/s} = \mathbf{19.2~\text{GB/s}} $$

{\small\textcolor{textMuted}{\faInfoCircle\hspace{2mm}Source: \href{https://docs.amd.com/r/en-US/ds962-u200-u250/Alveo-Product-Details}{AMD Alveo U200 Data Sheet (DS962)}}}
\end{solutionbox}

\begin{solutionbox}
{\color{purpleAccent}\faCalculator}\hspace{2mm}\textbf{Kernel Bandwidth Demand vs. Hardware Supply}

\vspace{2mm}

\textbf{V3 Architecture Demand:}

With \texttt{\#pragma HLS DATAFLOW}, all three stages execute \textbf{simultaneously}:
\begin{itemize}[label=\textcolor{accentBlue}{\faAngleRight}, leftmargin=5mm, itemsep=1mm]
    \item \texttt{compute\_diff\_wide}: Reading from \texttt{gmemA} (Image A)
    \item \texttt{compute\_diff\_wide}: Reading from \texttt{gmemB} (Image B)
    \item \texttt{write\_result\_wide}: Writing to \texttt{gmemC} (Image C)
\end{itemize}
...
\vspace{3mm}
{\color{dangerRed}\faExclamationCircle}\hspace{2mm}\textbf{Hardware Supply:} In the default configuration, all three AXI bundles (\texttt{gmemA}, \texttt{gmemB}, \texttt{gmemC}) are mapped to a \textbf{single DDR bank}.

\vspace{2mm}
\textbf{The ``Traffic Jam'' Result:}
$$ \text{Ratio} = \frac{\text{Demand}}{\text{Supply}} = \frac{57.6~\text{GB/s}}{19.2~\text{GB/s}} = \mathbf{3\times} $$
$$ \text{Actual Time} = 3.5~\mu s \times 3 \approx \mathbf{10.5~\mu s} $$

This aligns closely with the observed \textbf{11 $\mu$s} execution time.
\end{solutionbox}

\begin{tipbox}
{\color{successGreen}\faLightbulb}\hspace{2mm}\textbf{Optimization Path:}

\vspace{2mm}

To achieve the theoretical $\sim$3.5 $\mu$s execution time on real hardware, map each AXI bundle to a \textbf{separate physical DDR bank}:
...
# connectivity.cfg
[connectivity]
sp=IMAGE_DIFF_POSTERIZE_1.A:DDR[0]
sp=IMAGE_DIFF_POSTERIZE_1.B:DDR[1]
sp=IMAGE_DIFF_POSTERIZE_1.C:DDR[2]
\end{lstlisting}

\vspace{2mm}
This eliminates memory contention by providing 3 $\times$ 19.2 GB/s = 57.6 GB/s of parallel bandwidth, matching the kernel's demand.
\end{tipbox}

\begin{solutionbox}
{\color{successGreen}\faCheckCircle}\hspace{2mm}\textbf{Verified: V3 Performance After Multi-Bank DDR Mapping}

\vspace{2mm}

After applying the \texttt{connectivity.cfg} configuration to map each AXI bundle to a separate DDR bank:

\vspace{2mm}

\begin{tabular}{@{}lrr@{}}
\textbf{Metric} & \textbf{Before (Single Bank)} & \textbf{After (Multi-Bank)} \\
\midrule
Clock Frequency & 300 MHz & 300 MHz \\
\rowcolor{lightBlueBg!55} Compute Unit Time & \textbf{11 $\mu$s} & \textbf{5 $\mu$s} \\
Kernel Execution Time & 16 $\mu$s & 9 $\mu$s \\
\rowcolor{lightBlueBg!55} Transfer Rate (MB/s) & 8868.2 & \textbf{18886.5} \\
\end{tabular}

\vspace{3mm}

{\color{accentBlue}\faChartLine}\hspace{2mm}\textbf{Key Observations:}

\begin{itemize}[label=\textcolor{accentBlue}{\faAngleRight}, leftmargin=5mm, itemsep=1mm]
    \item \textbf{2.2$\times$ faster CU time:} Eliminating memory contention reduced compute unit time from 11 $\mu$s to \textbf{5 $\mu$s}, approaching the theoretical 3.5 $\mu$s.
    \item \textbf{2.1$\times$ higher transfer rate:} Kernel transfer rate increased from 8868.2 MB/s to \textbf{18886.5 MB/s}, nearly saturating the per-bank bandwidth.
    \item \textbf{Host transfers unchanged:} Host-to-device and device-to-host rates remain $\sim$3 MB/s (PCIe emulation limited).
\end{itemize}

{\small\textcolor{textMuted}{\faInfoCircle\hspace{2mm}The remaining gap between 5 $\mu$s and theoretical 3.5 $\mu$s is attributed to AXI protocol overhead and pipeline fill/drain latency.}}
\end{solutionbox}

% ---------------------------------------------------------------------------
% V2 ANALYSIS (The Partial Bottleneck)
% ---------------------------------------------------------------------------

\begin{solutionbox}
{\color{warningOrange}\faExclamationTriangle}\hspace{2mm}\textbf{V2 Performance Analysis: Partial Bottleneck}

\vspace{2mm}

\begin{tabular}{@{}lr@{}}
\textbf{Theoretical:} & $3104 \div 300\text{ MHz} \approx \mathbf{10.3~\mu s}$ \\
\textbf{Measured:} & $\mathbf{15~\mu s}$ (50\% slower) \\
\end{tabular}

\vspace{2mm}

\textbf{Root Cause:} Stage 1 reads A and B simultaneously, demanding 38.4 GB/s but limited to 19.2 GB/s. This 2$\times$ throttling on Stage 1 alone explains the gap:

$$ \text{Total} = \underbrace{6.8~\mu s}_{\text{Stage 1 (2$\times$)}} + \underbrace{3.5~\mu s}_{\text{Stage 2}} + \underbrace{3.4~\mu s}_{\text{Stage 3}} = 13.7~\mu s \approx \textbf{15}~\mu s $$

{\small\textcolor{textMuted}{\faLightbulb\hspace{1mm}V2 is only partially bottlenecked because Stages 2-3 operate within bandwidth limits.}}
\end{solutionbox}
...
\begin{tcolorbox}[mybox]
\faFlagCheckered\hspace{2mm}This laboratory exercise explored three architectural approaches for implementing an image processing accelerator with difference, posterization, and sharpening:

\begin{itemize}[leftmargin=5mm, itemsep=2mm]
    \item \textbf{V1 (Sequential + 2D Buffers):} Simplest implementation with pixel-by-pixel filter access. The 1-pixel/cycle filter stage creates a \textbf{97\% bottleneck}, resulting in 230 $\mu$s execution time.
    
    \item \textbf{V2 (Sequential + Line Buffers):} Achieves 64 pixels/cycle in all stages via sliding window. Stage 1's simultaneous A/B reads cause \textbf{2$\times$ memory contention} (10.3 $\mu$s $\rightarrow$ 15 $\mu$s).
    
    \item \textbf{V3 (Dataflow + Streams):} Maximum parallelism with concurrent stage execution. Default single-bank config caused \textbf{3$\times$ contention} (11 $\mu$s); multi-bank DDR mapping achieved \textbf{5 $\mu$s}.
\end{itemize}
\end{tcolorbox}

\begin{solutionbox}
{\color{successGreen}\faCheckCircle}\hspace{2mm}\textbf{Key Finding: Memory Optimization Verified}

\vspace{2mm}

Both V2 and V3 are \textbf{memory-bound}. Mapping AXI bundles to separate DDR banks via \texttt{connectivity.cfg} eliminated contention for V3:

\vspace{2mm}

\begin{tabular}{@{}lccc@{}}
\textbf{Version} & \textbf{Theoretical} & \textbf{Single-Bank} & \textbf{Multi-Bank} \\
\midrule
V2 & 10.3 $\mu$s & 15 $\mu$s & -- \\
V3 & 3.5 $\mu$s & 11 $\mu$s & \textbf{5 $\mu$s} \\
\end{tabular}

\vspace{2mm}
{\small\textcolor{textMuted}{\faLightbulb\hspace{1mm}V3's transfer rate doubled from 8868 MB/s to 18886 MB/s with multi-bank mapping.}}
\end{solutionbox}
...
\subsection{Single-Bank DDR Configuration (Default)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{./assets/SystemDiagram_v3_default.png}
    \caption{V3 kernel system diagram with default memory mapping. All three AXI master ports (\texttt{gmemA}, \texttt{gmemB}, \texttt{gmemC}) are mapped to a single DDR bank, creating a memory bandwidth bottleneck.}
    \label{fig:v3_old}
\end{figure}

\vspace{5mm}

\subsection{Multi-Bank DDR Configuration (Optimized)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{./assets/SystemDiagram_v3_optimized.png}
    \caption{V3 kernel system diagram with multi-bank DDR mapping via \texttt{connectivity.cfg}. Each AXI master port is mapped to a separate DDR bank, eliminating memory contention.}
    \label{fig:v3_new}
\end{figure}
