...
# connectivity.cfg
[connectivity]
sp=IMAGE_DIFF_POSTERIZE.gmemA:DDR[0]
sp=IMAGE_DIFF_POSTERIZE.gmemB:DDR[1]
sp=IMAGE_DIFF_POSTERIZE.gmemC:DDR[2]
\end{lstlisting}
...
\begin{solutionbox}
{\color{warningOrange}\faExclamationTriangle}\hspace{2mm}\textbf{V2 Performance Analysis: The Partial Bottleneck}

\vspace{2mm}
V2 also exhibits a performance gap, despite its sequential nature:

\begin{tabular}{@{}lr@{}}
\textbf{Theoretical Time:} & $3104 \div 300\text{ MHz} \approx \mathbf{10.3~\mu s}$ \\
\textbf{Measured Time:} & $\mathbf{15~\mu s}$ \\
\end{tabular}

\vspace{2mm}
\textbf{Why the 50\% slowdown?}
While the stages run sequentially, \textbf{Stage 1} still requires double the available bandwidth:

\begin{enumerate}[label=\textcolor{accentBlue}{\arabic*.}, leftmargin=6mm, itemsep=1mm]
    \item \textbf{Stage 1 (Difference \& Posterize):} Reads A and B simultaneously.
    \begin{itemize}[label=$\rightarrow$]
        \item Demand (38.4 GB/s) $>$ Supply (19.2 GB/s).
        \item \textbf{Result:} Throttled by \textbf{2x} ($3.4~\mu s \rightarrow \mathbf{6.8~\mu s}$).
    \end{itemize}
    
    \item \textbf{Stage 2 (Filter):} Reads/Writes local BRAM only.
    \begin{itemize}[label=$\rightarrow$]
        \item No DDR access. \textbf{Result:} Full speed ($\mathbf{3.5~\mu s}$).
    \end{itemize}
    
    \item \textbf{Stage 3 (Memory Write):} Writes C only.
    \begin{itemize}[label=$\rightarrow$]
        \item Demand equals Supply (19.2 GB/s). \textbf{Result:} Full speed ($\mathbf{3.4~\mu s}$).
    \end{itemize}
\end{enumerate}
...
\begin{solutionbox}
{\color{secondaryTeal}\faCalculator}\hspace{2mm}\textbf{Final Calculation}

\vspace{2mm}

Adding up the actual time for all sequential stages:

$$ \text{Total Time} = \underbrace{6.8~\mu s}_{\text{Stage 1 (Throttled)}} + \underbrace{3.5~\mu s}_{\text{Stage 2}} + \underbrace{3.4~\mu s}_{\text{Stage 3}} = \mathbf{13.7~\mu s} $$

\vspace{2mm}

The calculated 13.7 $\mu$s is very close to the measured \textbf{15 $\mu$s}. The remaining $\sim$1.3 $\mu$s accounts for kernel launch overhead and pipeline flushing.

\vspace{3mm}

{\color{accentBlue}\faLightbulb}\hspace{2mm}\textbf{Key Insight:} V2 is $\sim$1.5$\times$ slower because \textit{only Stage 1} tries to do 2 things at once (Read A + Read B), while the other stages operate within bandwidth limits.
\end{solutionbox}
...
\begin{itemize}[leftmargin=5mm, itemsep=2mm]
    ...
    \item \textbf{V2 (Sequential + Line Buffers):} Achieves 64 pixels/cycle in all stages via sliding window. However, Stage 1's simultaneous A/B reads cause \textbf{2$\times$ memory contention}, inflating execution from theoretical 10.3 $\mu$s to 15 $\mu$s.
    
    \item \textbf{V3 (Dataflow + Streams):} Maximum parallelism with concurrent stage execution. All 3 AXI ports active simultaneously create \textbf{3$\times$ memory contention}, limiting performance to 11 $\mu$s instead of theoretical 3.5 $\mu$s.
\end{itemize}
\end{tcolorbox}

\begin{warningbox}
{\color{warningOrange}\faExclamationTriangle}\hspace{2mm}\textbf{Key Finding: Memory-Bound Performance}

\vspace{2mm}

Both V2 and V3 are \textbf{memory-bound}, not compute-bound. Despite achieving near-optimal HLS latencies, actual execution times are limited by DDR bandwidth when multiple AXI ports share a single memory bank

\vspace{2mm}

\begin{tabular}{@{}lcccc@{}}
\textbf{Version} & \textbf{Theoretical} & \textbf{Measured} & \textbf{Bottleneck} & \textbf{Cause} \\
\midrule
V2 & 10.3 $\mu$s & 15 $\mu$s & 1.5$\times$ & Stage 1: 2 reads \\
V3 & 3.5 $\mu$s & 11 $\mu$s & 3$\times$ & DATAFLOW: 3 ports \\
\end{tabular}

\vspace{3mm}

{\color{successGreen}\faLightbulb}\hspace{2mm}\textbf{Optimization Path:} Map each AXI bundle to separate DDR banks via \texttt{connectivity.cfg} to eliminate contention and achieve theoretical performance.
\end{warningbox}
