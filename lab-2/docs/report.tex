...
\begin{table}[H]
    ...
    \begin{tabular}{@{}l c c c@{}}
        ...
        \rowcolor{lightBlueBg!55} Compute Unit Time ($\mu$s) & \textbf{230} & \textbf{15} & \textbf{11} \\
        Kernel Execution Time ($\mu$s) & \textbf{233} & \textbf{28} & \textbf{16} \\
        \bottomrule[1.5pt]
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}l c@{}}
        \toprule[1.5pt]
        \textbf{\textcolor{primaryBlue}{Metric}} & \textbf{\textcolor{primaryBlue}{Value}} \\
        \midrule[0.8pt]
        \multicolumn{2}{c}{\textbf{Host to Device}} \\
        \midrule[0.8pt]
        Transfer Count & \textbf{1} \\
        \rowcolor{lightBlueBg!55} Transfer Rate (MB/s) & \textbf{3.153} \\
        Avg Size (Bytes) & \textbf{131072} \\
        \midrule[0.8pt]
        \multicolumn{2}{c}{\textbf{Device to Host}} \\
        \midrule[0.8pt]
        Transfer Count & \textbf{1} \\
        \rowcolor{lightBlueBg!55} Transfer Rate (MB/s) & \textbf{1.422} \\
        Avg Size (Bytes) & \textbf{65536} \\
        \bottomrule[1.5pt]
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}p{0.35\textwidth} c c c@{}}
        ...
        \rowcolor{lightBlueBg!55} FPGA Exec Time ($\mu$s) & \textbf{233} & \textbf{28} & \textbf{16} \\
...
\begin{solutionbox}
{\color{warningOrange}\faExclamationTriangle}\hspace{2mm}\textbf{Important: Clock Frequency Mismatch}

The HLS synthesis report uses a \textbf{10 ns target period (100 MHz)}, but the Vitis system actually runs the kernel at \textbf{300 MHz}. This explains the apparent discrepancy between HLS estimates and measured execution times:

\vspace{2mm}
\begin{tabular}{@{}l r r r r@{}}
\toprule
\textbf{Version} & \textbf{Latency} & \textbf{HLS @ 100MHz} & \textbf{IDE @ 300MHz} & \textbf{Measured} \\
\midrule
V1 & 67,632 cycles & 676 $\mu$s & 225 $\mu$s & \textbf{233 $\mu$s} \\
V2 & 3,104 cycles & 31 $\mu$s & 10.3 $\mu$s & \textbf{28 $\mu$s} \\
V3 & 1,048 cycles & 10.5 $\mu$s & 3.5 $\mu$s & \textbf{16 $\mu$s} \\
\bottomrule
\end{tabular}

\vspace{2mm}
{\small\textcolor{textMuted}{\faInfoCircle\hspace{2mm}Source: \texttt{hw\_link $\rightarrow$ Emulation-HW $\rightarrow$ binary\_container $\rightarrow$ Link Summary}. Target: 300 MHz, Estimated: \textbf{411 MHz}.}}
\end{solutionbox}

\begin{solutionbox}
{\color{accentBlue}\faQuestionCircle}\hspace{2mm}\textbf{Why Do V2/V3 Still Show Higher Measured Times?}

While V1's measured time (233 $\mu$s) closely matches the 300 MHz prediction (225 $\mu$s), V2 and V3 show larger gaps. This is because:

\begin{itemize}[label=\textcolor{accentBlue}{\faAngleRight}, leftmargin=5mm, itemsep=1.5mm]
    \item \textbf{Fixed Overhead Dominance:} Kernel launch, AXI setup, and memory initialization add $\sim$10--15 $\mu$s of constant overhead. For V1 (225 $\mu$s compute), this is only 5\%. For V3 (3.5 $\mu$s compute), it dominates.
    \item \textbf{Memory Bandwidth Bottleneck:} V2 and V3's faster compute exposes DDR access latency as the limiting factor.
    \item \textbf{Stream/Dataflow Overhead:} V3's HLS streams have synchronization costs not captured in cycle estimates.
\end{itemize}

\vspace{2mm}
{\small\textcolor{textMuted}{\faLightbulb\hspace{2mm}For larger images, the compute-to-overhead ratio improves, and measured times would approach the 300 MHz predictions.}}
\end{solutionbox}
