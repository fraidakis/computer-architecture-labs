...
#pragma HLS INTERFACE s_axilite port=return bundle=control  // start/done/idle
  ...
  unsigned int v1_buffer[BUFFER_SIZE];   // Local memory to store vector1 - 1024 × 4 bytes = 4 KB BRAM
  unsigned int v2_buffer[BUFFER_SIZE];   // Local memory to store vector2 - 1024 × 4 bytes = 4 KB BRAM
  unsigned int vout_buffer[BUFFER_SIZE]; // Local Memory to store result - 1024 × 4 bytes = 4 KB BRAM

  // Per iteration of this loop perform BUFFER_SIZE vector additions
